 ...
 *     0.99996 -            - bracket #include <malloc.h> for macintosh
 * compilation
 *     0.99995 -            - avoid alias-optimization issue in float-to-int
 * conversion
 * 
 ...
 *  them from multiple threads at the same time. However, you can have
 * multiple
...
/* create a vorbis decoder by passing in the initial data block containing the
 * ogg&vorbis headers (you don't need to do parse them, just provide the first
 * N bytes of the file--you're told if it's not enough, see below) on success,
 * returns an stb_vorbis *, does not set error, returns the amount of data
 * parsed/consumed on this call in *datablock_memory_consumed_in_bytes; on
 * failure, returns NULL on error and sets *error, does not change
 **datablock_memory_consumed
 * if returns NULL and *error is VORBIS_need_more_data, then the input block
 * was incomplete and you need to pass in a larger block from the start of the
 *file
 */
extern int stb_vorbis_decode_frame_pushdata(stb_vorbis *f, unsigned char *datablock, int datablock_length_in_bytes,
	/* place to write number of float * buffers */ int *channels,
	/* place to write float ** array of float * buffers*/ float ***output,
	/* place to write number of output samples */ int *samples);
 ...
 *  the _current_ seek point (ftell). on failure, returns NULL and sets
 * *error.
...
/* create an ogg vorbis decoder from an open FILE *, looking for a stream at
 *  the _current_ seek point (ftell); the stream will be of length 'len'
 * bytes.
 ...
 * channels,
 *     and stereo R be the sum of all R and center channels (channel
 * assignment from the
 *     vorbis spec).
 *         M    N       output
 ...
 * requested
 *  at the end of the file. If there are no more samples in the file, returns
 * 0.
 */
#ifndef STB_VORBIS_NO_INTEGER_CONVERSION
...
#endif
/* gets num_samples samples, not necessarily on a frame boundary--this requires
 *  buffering so you have to supply the buffers. Applies the coercion rules
 * above
 ...
 */
#endif /* ifndef STB_VORBIS_NO_PULLDATA_API */
...
struct stb_vorbis {
	...
	int					last_seg_which; /* what was the segment number of the
	                                     * last seg? */
	uint32				acc;
	int					valid_bits;
	int					packet_bytes;
 ...
 *  @OPTIMIZE: called multiple times per-packet with "constants"; move to
 * setup*/
...
/* zlib & jpeg huffman tables assume that the output symbols can either be
 * arbitrarily arranged, or have monotonically increasing frequencies--they
 * rely on the lengths being sorted; this makes for a very simple generation
 * algorithm. vorbis allows a huffman table with non-sorted lengths. This
 * requires a more sophisticated construction, since symbols in order do not
 * map to huffman codes "in order".
 */
...
/* accelerated huffman table allows fast O(1) match of all symbols of
 * length <= STB_VORBIS_FAST_HUFFMAN_LENGTH */
...
/*called from setup only, once per code book (formula implied by
 *specification)*/
...
#endif /* ifndef STB_VORBIS_DIVIDES_IN_CODEBOOK */
 ...
 * note that you must produce bit-identical output to decode correctly; this
 * specific sequence of operations is specified in the spec (it's drawing
 * integer-quantized frequency-space lines that the encoder expects to be
 * exactly the same) ... also, isn't the whole point of Bresenham's algorithm
 * to NOT have to divide in the setup? sigh.
 */
#ifndef STB_VORBIS_NO_DEFER_FLOOR
...
void inverse_mdct_slow(float *buffer, int n){
	...
	for (i = 0; i < n; ++i) {
		...
		for (j = 0; j < n2; ++j)
			/* formula from paper:
			 * acc += n/4.0f * x[j] * (float) cos(M_PI / 2 / n * (2 * i + 1 +
			 * n/2.0)*(2*j+1));
			 * formula from wikipedia
			 * acc += 2.0f / n2 * x[j] * (float) cos(M_PI/n2 * (i + 0.5 +
			 * n2/2)*(j + 0.5));
			 * these are equivalent, except the formula from the paper inverts
			 * the multiplier!
			 * however, what actually works is NO MULTIPLIER!?!
			 * acc += 64 * 2.0f / n2 * x[j] * (float) cos(M_PI/n2 * (i + 0.5 +
			 * n2/2)*(j + 0.5));
			 */
...
void inverse_mdct_slow(float *buffer, int n, vorb *f, int blocktype){
	...
	dct_iv_slow(temp, n2);                                  /* returns -c'-d,
	                                                         * a-b' */
...
void inverse_mdct_naive(float *buffer, int n){
	 ...
	 * (by first figuring out what made inverse_mdct_slow work); then matching
	 * that here
	 * (probably vorbis encoder premultiplies by n or n/2, to save it on the
	 * decoder?)
	 */
	s = 0.5; /* theoretically would be n4 */

	/* [[[ note! the s value of 0.5 is compensated for by the B[] in the current
	 * code,
	...
	for (i = 0; i < f->channels; ++i) {
		...
		if (f->floor_types[floor] == 0) {
			...
			if (get_bits(f, 1)) {
				...
				if (f->valid_bits == INVALID_BITS) goto error; /* behavior
				                                                * according to
				                                                * spec */
	...
	for (i = 0; i < f->codebook_count; ++i) {
		...
		if (c->lookup_type > 0) {
			...
			for (j = 0; j < (int)c->lookup_values; ++j) {
				...
				if (q == EOP) {
					setup_temp_free(f, mults, sizeof(mults[0]) * c->lookup_values); return error(f, VORBIS_invalid_setup);
				}
				mults[j] = q;
			}
			...
			if (c->lookup_type == 1) {
				...
				if (c->multiplicands == NULL) {
					setup_temp_free(f, mults, sizeof(mults[0]) * c->lookup_values); return error(f, VORBIS_outofmem);
				}
				...
				for (j = 0; j < len; ++j) {
					...
					for (k = 0; k < c->dimensions; ++k) {
						...
						/* in this case (and this case only) we could pre-expand
						 * c->sequence_p,
						 * and throw away the decode logic for it; have to ALSO
						 * do
	...
	for (i = 0; i < f->page_crc_tests; ) {
		...
		if (f->scan[i].bytes_left == 0) {
			...
			if (f->scan[i].crc_so_far == f->scan[i].goal_crc) {
				...
				data_len = n + m;                       /* consumption amount is
				                                         * wherever that scan
				                                         * ended */
				f->page_crc_tests = -1;                 /* drop out of page scan
				                                         * mode */
				f->previous_length = 0;                 /*
				                                         * decode-but-don't-output
				                                         * one frame */
				f->next_seg = -1;                       /* start a new page */
				f->current_loc = f->scan[i].sample_loc; /* set the current
				                                         * sample location */
...
static int vorbis_seek_frame_from_page(stb_vorbis *f, uint32 page_start, uint32 first_sample, uint32 target_sample, int fine){
	...
	if (fine) {
		/* so if we're doing sample accurate streaming, we want to go ahead and
		 * decode it! */
...
static int vorbis_seek_base(stb_vorbis *f, unsigned int sample_number, int fine){
	...
	if (sample_number < f->p_first.last_decoded_sample) {
		...
		while (p[0].page_end < p[1].page_start) {
			...
			end_offset = p[1].after_previous_page_start; /* an address known to
			                                              * seek to page p[1] */
...
void stb_vorbis_seek_start(stb_vorbis *f){
	if (IS_PUSH_MODE(f)) {
		error(f, VORBIS_invalid_api_mixing); return;
	}
